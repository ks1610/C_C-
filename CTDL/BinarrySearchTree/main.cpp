...
Node* treeFindMin(Node * tree){
    if(tree == NULL)
        throw std::runtime_error("Tree is empty.");
    return (tree->left == NULL) ? tree : treeFindMin(tree->left);
}

Node* treeFindMax(Node * tree){
    if(tree == NULL)
        throw std::runtime_error("Tree is empty.");
    return (tree->right == NULL) ? tree : treeFindMax(tree->right);
}

void treeDeleteX(Node*& tree, int x) {
    if (tree == NULL) {
    ...
    if (x < tree->maSV) {
        ...
        treeDeleteX(tree->left, x);
    } else if (x > tree->maSV) {
        // Delete from the right subtree
        treeDeleteX(tree->right, x);
    } else {
        // Node found
        if (tree->left != NULL && tree->right != NULL) {
            // Node has two children
            Node* minRight = treeFindMin(tree->right);
            tree->maSV = minRight->maSV;
            tree->hoTen = minRight->hoTen;
            treeDeleteX(tree->right, minRight->maSV); // Delete the duplicate from the right subtree
        } else {
...
int main()
{
	...
	*/
    ...
    //Tim Gia Tri Nho Nhat Lon Nhat Trong Cay
	std::cout<<std::endl;
    try {
        Node* foundMin = treeFindMin(tree.root);
        std::cout << "Min: " << foundMin->maSV << " - " << foundMin->hoTen << std::endl;
    } catch (const std::exception& e) {
        std::cerr << e.what() << std::endl;
    }
    try {
        Node* foundMax = treeFindMax(tree.root);
        std::cout << "Max: " << foundMax->maSV << " - " << foundMax->hoTen << std::endl;
    } catch (const std::exception& e) {
        std::cerr << e.what() << std::endl;
    }

    //Xoa Nut
    treeDeleteX(tree.root, 5);
